...
/*
    THE STORK PROJECT
    Due date: 23 January 2027

    Pregnancy information in this tracker is general and approximate.
    Advice from the maternity team should always take priority.
*/
...
const missionStorageKey = "storkProjectWeeklyMissions";
const memoryStorageKey = "storkProjectWeeklyMemories";
const viewedWeekStorageKey = "storkProjectLastViewedWeek";
...
const fallbackBirdFacts = [
    "Birds are living dinosaurs: modern birds evolved from theropod dinosaurs.",
    "Feathers probably evolved before powered flight and originally helped with insulation or display.",
    "Corvids, including crows and magpies, demonstrate advanced problem-solving abilities.",
    "Many migratory birds navigate using a combination of landmarks, the Sun, stars and Earth’s magnetic field.",
    "A bird’s respiratory system moves air through the lungs in a largely one-way flow, supported by air sacs.",
    "Some birds sleep with one brain hemisphere at a time, keeping the other side alert.",
    "Owls cannot move their eyes substantially within their sockets, so they turn their heads instead.",
    "The Arctic tern completes one of the longest known annual migrations of any animal.",
    "Bird bones are not simply hollow; many contain reinforcing internal structures.",
    "Pigeons can distinguish visual patterns and have performed surprisingly complex categorisation tasks."
];
...
const trimesterBadge =
    document.getElementById("trimesterBadge");

const sizeEmoji =
    document.getElementById("sizeEmoji");
...
const birdEmoji =
    document.getElementById("birdEmoji");

const birdComparisonName =
    document.getElementById("birdComparisonName");

const birdComparisonText =
    document.getElementById("birdComparisonText");

const scienceFacts =
    document.getElementById("scienceFacts");

const birdFact =
    document.getElementById("birdFact");

const mumFact =
    document.getElementById("mumFact");

const dadFact =
    document.getElementById("dadFact");

const missionHeading =
    document.getElementById("missionHeading");

const missionDescription =
    document.getElementById("missionDescription");

const weeklyMissionCheckbox =
    document.getElementById("weeklyMissionCheckbox");

const daysRemainingElement =
    document.getElementById("daysRemaining");

const countdownProgressFill =
    document.getElementById("countdownProgressFill");

const countdownProgress =
    document.querySelector(".countdown-progress");

const pregnancyPercentage =
    document.getElementById("pregnancyPercentage");

const storkTraveller =
    document.getElementById("storkTraveller");

const storkMessage =
    document.getElementById("storkMessage");

const weekStatistic =
    document.getElementById("weekStatistic");

const daysCompleted =
    document.getElementById("daysCompleted");

const daysRemainingStatistic =
    document.getElementById("daysRemainingStatistic");

const trimesterStatistic =
    document.getElementById("trimesterStatistic");

const starSign =
    document.getElementById("starSign");

const milestoneIcon =
    document.getElementById("milestoneIcon");
...
const weekTimeline =
    document.getElementById("weekTimeline");

const weeklyMemory =
    document.getElementById("weeklyMemory");

const saveMemoryButton =
    document.getElementById("saveMemoryButton");

const memorySavedMessage =
    document.getElementById("memorySavedMessage");

const celebrationCanvas =
    document.getElementById("celebrationCanvas");

const weekCelebrationModal =
    document.getElementById("weekCelebrationModal");
...
const closeCelebrationButton =
    document.getElementById("closeCelebrationButton");

/* --------------------------------------------------
   DATE CALCULATIONS
-------------------------------------------------- */

function getPregnancyStartDate() {
    return new Date(
        DUE_DATE.getTime() -
        PREGNANCY_LENGTH_DAYS * DAY_IN_MS
    );
}
...
function calculatePregnancy() {
    ...
    const pregnancyStart = getPregnancyStartDate();

    const rawDaysCompleted = Math.floor(
        (today - pregnancyStart) / DAY_IN_MS
    );

    const rawDaysRemaining = Math.ceil(
        (DUE_DATE - today) / DAY_IN_MS
    );

    const completed = Math.max(
        0,
        Math.min(PREGNANCY_LENGTH_DAYS, rawDaysCompleted)
    );
    ...
    const percentage = Math.max(
        ...
        Math.min(
            100,
            Math.round(
                (completed / PREGNANCY_LENGTH_DAYS) * 100
            )
        )
    );
...
function getWeekInformation(week) {
    if (weekData[week]) {
        return weekData[week];
    }

    const nearestWeek = Object.keys(weekData)
        .map(Number)
        .reduce((closest, availableWeek) => {
            return Math.abs(availableWeek - week) <
                Math.abs(closest - week)
                ? availableWeek
                : closest;
        }, 4);

    return weekData[nearestWeek];
}
...
function renderApp() {
    const pregnancy = calculatePregnancy();
    const information = getWeekInformation(pregnancy.week);
    const trimester = getTrimester(pregnancy.week);
    ...
    renderScienceFacts(information);
    renderParentNotes(information);
    renderMission(pregnancy.week, information);

    renderCountdown(pregnancy);
    renderStorkRoute(pregnancy);
    renderStatistics(pregnancy, trimester);
    renderMilestone(pregnancy.week);
    renderTimeline(pregnancy.week);
    renderMemory(pregnancy.week);

    checkForNewWeek(pregnancy.week);
}
...
function renderWeekHeader(
    ...
    birdFact.textContent =
        ...
        fallbackBirdFacts[
            pregnancy.week %
            fallbackBirdFacts.length
        ];
}

function renderScienceFacts(information) {
    scienceFacts.innerHTML =
        information.facts
...
function renderParentNotes(information) {
    mumFact.textContent = information.mumFact;
    dadFact.textContent = information.dadFact;
}

function renderMission(week, information) {
    missionHeading.textContent =
        information.mission;

    missionDescription.textContent =
        information.missionDescription;

    const completedMissions =
        readStoredObject(missionStorageKey);

    weeklyMissionCheckbox.checked =
        Boolean(completedMissions[week]);
}

function renderCountdown(pregnancy) {
    daysRemainingElement.textContent =
        pregnancy.remaining.toLocaleString("en-GB");

    daysRemainingStatistic.textContent =
        pregnancy.remaining.toLocaleString("en-GB");

    daysCompleted.textContent =
        pregnancy.completed.toLocaleString("en-GB");
    ...
    countdownProgress.setAttribute(
        "aria-valuenow",
        String(pregnancy.percentage)
    );

    if (pregnancy.overdue) {
        pregnancyPercentage.textContent =
            "The estimated due date has passed — awaiting updated field observations.";
    } else {
        pregnancyPercentage.textContent =
            `${pregnancy.percentage}% of the estimated 40-week migration complete`;
    }
}

function renderStorkRoute(pregnancy) {
    storkTraveller.style.left =
        `${pregnancy.percentage}%`;

    if (pregnancy.percentage < 25) {
        storkMessage.textContent =
            "The stork has received the assignment and is reviewing the initial flight plan.";
    } else if (pregnancy.percentage < 50) {
        storkMessage.textContent =
            "The stork is airborne and carrying an increasingly complex scientific dataset.";
    } else if (pregnancy.percentage < 75) {
        storkMessage.textContent =
            "The stork has passed the halfway migration marker.";
    } else if (pregnancy.percentage < 95) {
        storkMessage.textContent =
            "The stork is approaching the delivery zone. Nest preparations are advised.";
    } else {
        storkMessage.textContent =
            "The stork is circling overhead. Delivery timing remains under Baby’s control.";
    }
}
...
function renderStatistics(
    ...
    weekStatistic.textContent =
        `${pregnancy.week} + ${pregnancy.weekDay}`;

    trimesterStatistic.textContent =
        trimester;

    starSign.textContent =
        getStarSign(DUE_DATE);
}
...
const milestones = [
    {
        week: 12,
        icon: "🔬",
        title: "First-trimester research complete",
        description:
            "The first major phase of development is approaching completion."
    },
    {
        week: 20,
        icon: "📡",
        title: "Halfway observation point",
        description:
            "The 20-week milestone and anatomy scan period are approaching."
    },
    {
        week: 24,
        icon: "🧠",
        title: "Major developmental milestone",
        description:
            "The developing baby continues substantial brain and lung development."
    },
    {
        week: 28,
        icon: "🪶",
        ...
        description:
            "The final major phase of the pregnancy migration is approaching."
    },
    {
        week: 32,
        icon: "🪹",
        title: "Nest preparation phase",
        description:
            "A useful time to check equipment, hospital plans and family logistics."
    },
    {
        week: 36,
        icon: "🧳",
        title: "Hospital bag readiness",
        description:
            "Key bags, notes and contact details should be readily accessible."
    },
    {
        week: 37,
        icon: "🐣",
        title: "Term pregnancy",
        description:
            "From 37 completed weeks, the pregnancy is generally described as term."
    },
    {
        week: 40,
        icon: "🕊️",
        title: "Estimated hatching week",
        description:
            "The estimated due date is approaching, although babies operate according to their own publication schedule."
    }
];

function renderMilestone(currentWeek) {
    const nextMilestone =
        milestones.find(
            milestone =>
    ...
    milestoneTitle.textContent =
        `Week ${nextMilestone.week}: ${nextMilestone.title}`;
...
weeklyMissionCheckbox.addEventListener(
    "change",
    () => {
        const pregnancy = calculatePregnancy();
        const completedMissions =
        ...
        completedMissions[pregnancy.week] =
            weeklyMissionCheckbox.checked;
        ...
        if (weeklyMissionCheckbox.checked) {
...
function renderMemory(week) {
    const memories =
        readStoredObject(memoryStorageKey);
...
saveMemoryButton.addEventListener(
    ...
    () => {
        ...
        memories[pregnancy.week] =
            weeklyMemory.value.trim();
        ...
        memorySavedMessage.textContent =
            `Week ${pregnancy.week} field note saved.`;
...
/* --------------------------------------------------
   NEW-WEEK CELEBRATION
-------------------------------------------------- */

function checkForNewWeek(currentWeek) {
    const lastViewedWeek =
        Number(
            localStorage.getItem(
                viewedWeekStorageKey
            )
        );

    if (
        lastViewedWeek &&
        currentWeek > lastViewedWeek
    ) {
        showWeekCelebration(currentWeek);
    }
...
function showWeekCelebration(week) {
    const information =
        getWeekInformation(week);

    celebrationTitle.textContent =
        `Week ${week} has hatched!`;

    celebrationMessage.textContent =
        `The latest field report compares Baby with a ${information.sizeName} and ${information.birdName}.`;

    weekCelebrationModal.hidden = false;
...
function closeCelebration() {
    weekCelebrationModal.hidden = true;
}
...
weekCelebrationModal.addEventListener(
    "click",
    event => {
        if (
...
const celebrationContext =
    celebrationCanvas.getContext("2d");

let celebrationPieces = [];
...
function resizeCelebrationCanvas() {
    ...
    celebrationContext.setTransform(
        ratio,
        0,
        0,
        ratio,
...
function launchFeatherCelebration(
    originX,
    originY,
    amount = 100
) {
...
function animateCelebration() {
    ...
    celebrationPieces =
        celebrationPieces.filter(
            piece => piece.life > 0
        );

    celebrationPieces.forEach(piece => {
        piece.sway += 0.08;
        ...
        celebrationContext.translate(
            piece.x,
            piece.y
        );

        celebrationContext.rotate(
            piece.rotation
        );

        celebrationContext.fillStyle =
            piece.colour;

        celebrationContext.beginPath();

        celebrationContext.ellipse(
            0,
            0,
            piece.width / 2,
            piece.height / 2,
